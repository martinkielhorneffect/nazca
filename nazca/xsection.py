...
class Material():
    ...
    def Neff(self, **kwargs):
        ...
        """
        if isinstance(self.__Neff, float):
             return self.__Neff
...
class Stack():
    ...
    def __init__(self, layers=None, etchdepth=0, background=None, name='', function=None, solver=None, view='topview'):
    ...
    def set(self, layers=None, etchdepth=0, function=None, solver=None, name=None, view=''):
        ...
        if name is None:
            self.name = 'stack-'.join(str(self.__ids))
        else:
            self.name = name

        if solver is None:
            if len(layers) == 1:
                self.__Neff = layers[0][0].Neff
                self.solver = None
            else:
                self.solver = slab.Slabsolver(name=self.name,
                    layers=layers, view=view)
                self.__Neff = self.solver.Neff
        else:
            self.solver = solver
            solver.view = view
            self.__Neff = solver.Neff
...
class Xsection():
    ...
    The XS is defined as a horizontal stack 'hstack' of one or more
    'vertical' layer stacks, 'vstack'.

    Example:
        3 vstacks in a hstack

    Each vstack has one or more layers, each layer has a material assigned to it.
    A vstack stack has a width attribute.
    ...
    def __init__(self, background=None, layers=None, name=''):
        ...
        self.name = name
        self.hstack = {}
        self.__stack_ids = count(0)
        self.layers = layers #epi
        self.background = background
        #waveguiding properties:
        #self.os = 0 # straight-bend offset
        #self.width = 0 # straight-bend offset
        #self.radius = 0 # straight-bend offset
    ...
    def add_vstack(self, layers=None, etchdepth=0, name='', function=None,
            solver=None):
        ...
        st = Stack(layers, etchdepth, self.background, name, function, solver,
            view='sideview')
...
#            print ('rib', i, ', function:', v.function, ', solver:', v.solver)
    ...
    def index_model(self, IM):
        ...
        self.__IM = IM
        self.Nrte = self.__IM.Nrte
        self.Nrtm = self.__IM.Nrtm
        self.Nbte = self.__IM.Nbte
        self.Nbtm = self.__IM.Nbtm
        self.Nste = self.__IM.Nste
        self.Nstm = self.__IM.Nstm
        self.dNRte = self.__IM.dNRte
        self.dNRtm = self.__IM.dNRtm
        self.Neff2Dte = self.__IM.Neff2Dte
        self.Neff2Dtm = self.__IM.Neff2Dtm
        self.oste = self.__IM.SBOte
        self.ostm = self.__IM.SBOtm
        return self.__IM
    ...
    def showGuide3D(self, draw_index=False):
        ...
        for i, (width,stack) in enumerate(zip(self.hstack['widths'],self.hstack['vstacks'])):
            ...
            for j, (h, mat) in enumerate(zip(stack.widths, stack.materials)):
                if draw_index:
                    intens = (mat.Neff()-self.__minNeff()) / (self.__maxNeff()-self.__minNeff())
                    col = (intens, intens, intens)
                    if col==(0,0,0): #do not draw white materials (air)
                        break
                else:

...
def Tp_MIR2p(length=70.6, width=39, pinwidth=None, offset=1.5,
    ...
    def cell():
        ...
        with nd.Cell(hashme=True) as C:
            ...
            C.groupname = groupname
            pdk.addBBmap(name, params=( OD_toStd))
            nd.Pin(name='a0', xs=xs['a0'], width=pinwidth['a0']).put(0, +offset, 180)
            #nd.Pin(name='b1', xs=xs['b1'], width=pinwidth['b1']).put(0, +offset, 180)
            nd.Pin(name='a1', xs=xs['a1'], width=pinwidth['a1']).put(0, -offset, 180)
...
def Tp_MMI1x2(length, width, pinwidth=None, offset=0,
    ...
    def cell():
        ...
        with nd.Cell(hashme=True) as C:
            ...
            pdk.addBBmap(name, params=(OD_toStd))
            nd.Pin(name='a0', xs=xs['a0'], width=pinwidth['a0']).put(0, 0, 180)
            nd.Pin(name='b0', xs=xs['b0'], width=pinwidth['b0']).put(length, +offset)
            nd.Pin(name='b1', xs=xs['b1'], width=pinwidth['b1']).put(length, -offset)

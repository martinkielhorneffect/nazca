...
class Font():
    ...
    """
    def __init__(self, fontfile=default_font, box_layer=None, box_buf=None):
        ...
        if not os.path.exists(ff):
            ...
            if not os.path.exists(ff):
                ...
                ff = os.path.join(modulepath, 'fonts', default_font+'.nft')
    ...
    def text(self, text="Text", height=50, layer=1, align='lb', box_layer=None,
             box_buf=None, instantiate=False):
        """Output a cell with text of specified dimensions and alignment.

        In case of an exclusion box, it will be scaled with the text height
        unless a box_buf value is provided, which represent an absolute
        buffer between the text field and the exclusion edge in um.
        ...
        id5 = md5(text + align.lower() + "{}{}{}".format(self.ff, height,
            layer))
        with Cell('text_{}_{}'.format(self.ff,id5),
                instantiate=instantiate) as txt:
            ...
            for line in text.split('\n'):
                ...
                for char in line:
                    ...
                    for poly in polys:
                        ...
                        if len(XY) > 3:
                            Polygon(points=XY, layer=layer).put(xpos, ypos, 0),
                    xpos = xpos + (w + self.spc) * scale
                ypos -= ls
            if box_layer is not None:
                Polygon(layer=box_layer, points=[(0, 0), (0, height+2*box_buf),
                    (bbx+2*box_buf, height+2*box_buf), (bbx+2*box_buf, 0)]).\
                        put(xpos0-box_buf, ypos0-box_buf, 0)

            Pin(name='b0', xs=None).put(xpos, ypos+ls, 0)
        return txt
...
def default_font(f=default_font):
    global fdef
    fdef = Font(f)
